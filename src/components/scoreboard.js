...
function Scoreboard() {
  ...
  console.log(
    "🚀 ~ file: scoreboard.js ~ line 35 ~ checkInvalid ~ invalid",
    invalid
  );
  ...
  const calculateHands = (e, idx) => {
    ...
    for (let ele in call) {
      if (call[ele] > -1 && call[ele] != "") {
        ...
        else sum = sum + (value && value != 0 ? parseInt(value) : 0);
      } else {
        if (ele == idx) {
          sum = sum + parseInt(value);
        }
    ...
    settotalSum(sum);
    console.log(
      "🚀 ~ file: scoreboard.js ~ line 79 ~ calculateHands ~ sum",
      sum
    );
    setmax(max);
      ...
      <Container>
            ...
            return (
              <Col>
                <input
                  ...
                  max={numberOfHands}
                  value={call[idx]}
                  onChange={(e) => calculateHands(e, idx)}
                ></input>
              </Col>
            );
          })}
        ...
        <Row>
          <Col className="totalCall">
            {call[nameOfPlayers.length - 1] != undefined &&
            call[nameOfPlayers.length - 1] != "" ? (
              <span className="totalCall">Total Call: {totalSum}</span>
            ) : (
              <>
                {totalSum <= numberOfHands &&
                call[nameOfPlayers.length - 2] >= 0 ? (
